...
public class SimpleRegion extends Object implements Serializable {
    ...
    public boolean isWithin(double longitude, double latitude, double distance) {
        switch (type) {
            ...
            case 1:
                /* return for bounding box */
                // TODO: This includes a square at the corners, when it should be a quarter circle.
                return (longitude <= points[2] + distance
                  && longitude >= points[0] - distance
                  && latitude <= points[3] + distance
    ...
    public Double distance(double longitude, double latitude, double distance, boolean evenWithin) {
        switch (type) {
            ...
            case 1:
                /* return for bounding box */
                // TODO: This includes a square at the corners, when it should be a
                // quarter circle.
                if (longitude <= points[2] + distance
                    && longitude >= points[0] - distance
                    && latitude <= points[3] + distance
                    && latitude >= points[1] - distance) {
                    return Math.min(
    ...
    private Double distancePolygon(double longitude, double latitude, double distance, boolean findShortestDistance, boolean evenIfInside) {
        ...
        for (int i = 2; i < len; i += 2) {
            ...
            //System.err.println("Segment ("+px+","+py+"), ("+qx+","+qy+")");
            ...
            //System.err.println("Segment ("+px+","+py+"), ("+qx+","+qy+"), r="+r+", dist="+dist);
        ...
        if (longitude <= bounding_box[1][0] && longitude >= bounding_box[0][0]
            ...
            for (i = 2; i < len; i += 2) {
                ...
                if ((SpatialUtil.convertLngToPixel(points[i]) > longitudePx) != segment) {
                    //lat value at line crossing > target point
                    y = (int) ((longitudePx - SpatialUtil.convertLngToPixel(points[i]))
                            * ((SpatialUtil.convertLatToPixel(points[i + 1]) - SpatialUtil.convertLatToPixel(points[i - 1]))
                            / (double) (SpatialUtil.convertLngToPixel(points[i]) - SpatialUtil.convertLngToPixel(points[i - 2])))
                            + SpatialUtil.convertLatToPixel(points[i + 1]));
